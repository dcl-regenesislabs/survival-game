...
const PLAYER_TRIGGER_SHADOW_LOCAL_POSITION = Vector3.create(0, 0.05, 0)
...
export class LobbyWorldPanel {
  ...
  constructor() {
    this.createPanel()
    this.createPlayerTriggerShadow()
    this.createButtons()
  ...
  private createPanel(): void {
    ...
    triggerAreaEventsSystem.onTriggerEnter(this.triggerEntity, (result) => {
      ...
      Transform.getMutable(this.triggerShadowEntity).scale = TRIGGER_SHADOW_SCALE_VISIBLE
    })
    triggerAreaEventsSystem.onTriggerExit(this.triggerEntity, (result) => {
      if (result.trigger?.entity !== engine.PlayerEntity) return
      console.log('[LobbyPanel] Player exited trigger area')
      Transform.getMutable(this.triggerShadowEntity).scale = Vector3.Zero()
    })
  }

  private createPlayerTriggerShadow(): void {
    Transform.create(this.triggerShadowEntity, {
      parent: engine.PlayerEntity,
      position: PLAYER_TRIGGER_SHADOW_LOCAL_POSITION,
      rotation: Quaternion.Identity(),
      scale: Vector3.Zero()
    })
